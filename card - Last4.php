...
if (empty($reshook)) {
	...
	if ($action == 'confirm_validate' && $confirm == 'yes' && $user->hasRight('ficheinter', 'creer')) {
					...
					if ($result > 0) {
						...
						if (is_array($lines)) {
							foreach ($lines as $line) {
								if (!in_array($line->id, $selectedLines)) continue;
	
								$product_type = ($line->product_type ? $line->product_type : Product::TYPE_PRODUCT);
	
								if ($prod->duration_value && $conf->global->FICHINTER_USE_SERVICE_DURATION) {
									switch ($prod->duration_unit) {
										case 'h': default: $mult = 3600; break;
										case 'd': $mult = 3600 * 24; break;
										case 'w': $mult = 3600 * 24 * 7; break;
										case 'm': $mult = (int) 3600 * 24 * (365 / 12); break;
										case 'y': $mult = 3600 * 24 * 365; break;
									}
									$duration = $prod->duration_value * $mult * $line->qty;
								}
	
								$desc = $line->product_ref . ' - ' . $label . '<br>';
								$desc .= dol_htmlentitiesbr($line->desc);
								$desc .= '<br>(' . $langs->trans('Quantity') . ': ' . $line->qty . ')';
        ...
        // Mise a jour d'une linea d'intervento
...
if ($action == 'create') {
    ...
    if ($socid > 0) {
        ...
        if (isModEnabled('project')) {
            ...
            if ($numprojet == 0) {
                print ' &nbsp; <a href="'.DOL_URL_ROOT.'/projet/card.php?socid='.$soc->id.'&action=create"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("AddProject").'"></spa[...]
            }
            print '</td></tr>';
        }
        ...
        if (isModEnabled('contract')) {
            ...
            if ($numcontrat == 0) {
                print ' &nbsp; <a href="'.DOL_URL_ROOT.'/contrat/card.php?socid='.$soc->id.'&action=create"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("AddContract").'"></s[...]
            }
            print '</td></tr>';
        }
        ...
        print ' <a href="'.DOL_URL_ROOT.'/societe/card.php?action=create&backtopage='.urlencode($_SERVER["PHP_SELF"].'?action=create').'"><span class="fa fa-plus-circle valignmiddle paddingleft" title=[...]
        print '</td></tr>';
        print '</table>';
